...
### 12.6 Telegram Bridge API Not Running

The Telegram Bridge (`telegram-bridge.service`) provides a REST API on port 9000 for sending/receiving Telegram messages outside the bot API. If it's not running:
...
# Check logs
sudo journalctl -u telegram-bridge -n 50
```

The nginx vhost `bridge-api.nhprince.dpdns.org` reverse-proxies to this service.
